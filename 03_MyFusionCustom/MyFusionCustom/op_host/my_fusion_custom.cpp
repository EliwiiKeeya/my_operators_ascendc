 ...
 ****************************************************************************************************/
#include "my_fusion_custom_tiling.h"
...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    MyFusionCustomTilingData tiling;
    if (cubeTiling.GetTiling(tiling.cubeTilingData) == -1) { // Get fusion tiling.
...
namespace ops {
class MyFusionCustom : public OpDef {
public:
    explicit MyFusionCustom(const char *name) : OpDef(name)
    {
        this->Input("a")
            .ParamType(REQUIRED)
